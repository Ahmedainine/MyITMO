...
public class Main {
	...
	public static void PrintArray(double[][] Array , String rounder)
	{
		for(double [] rows : Array)
		{
			for(double row : rows)
			{
				System.out.printf(rounder, row);
			}
			System.out.println();
		}
	}
    ...
    public static double TheHardThings(long l , double x)
    {
    	...
    	{
    		return Math.pow(Math.sin(Math.tan(Math.pow((1.0/4)/x, x))), Math.log(Math.abs(Math.asin(Math.pow(Math.E, -1*Math.abs(x))))));
    	}
    }
